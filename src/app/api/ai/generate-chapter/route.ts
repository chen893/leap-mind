...
* **结构化与完整性:** 将核心知识分解为2-4个关键点，进行明确分割。请确保知识点全面、深入，覆盖本章主题所需的所有核心概念，无重要遗漏。
${teachingFocusInstruction}
...
* **任务格式:** 采用“**场景描述 -> 任务指令 -> 参考步骤 -> 参考答案**”的结构。其中，“参考答案”部分必须提供一个完整、正确且带有详细解释的实现，以便学习者对照学习。
...
# 输出格式要求
* **直接开始:** 你的回答必须直接以"### 第一部分：引言 (Introduction)"作为开头，且必须独占第一行。
* **严禁寒暄:** 在引言部分前，绝对不允许出现任何形式的引言、确认性语句（如："好的，这是您要的内容…"、"好的，作为一名经验丰富的教学设计师与领域专家"）或任何其他文字。
\
