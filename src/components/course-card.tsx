...
export function CourseCard({
  ...
  const progressPercentage = progress
    ? (progress.unlockedChapters.length / (course?.chapters?.length || 1)) * 100
    : 0;

  return (
    <Card className="group hover:-translate-y-2 relative overflow-hidden border-0 bg-white shadow-md transition-all duration-500 hover:shadow-2xl">
      {/* 悬浮效果背景 */}
      ...
      <CardHeader className="relative h-[180px]">
        <div className="flex items-start justify-between">
          <div className="flex-1">
            <CardTitle className="line-clamp-2 font-bold text-gray-900 text-lg transition-colors duration-300 group-hover:text-blue-700">
              {course.title}
            </CardTitle>
            <CardDescription className="mt-3 line-clamp-2 text-gray-600 leading-relaxed">
              {course.description}
            </CardDescription>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Avatar className="h-8 w-8 ring-2 ring-gray-100 transition-all duration-300 group-hover:ring-blue-200">
              ...
              <AvatarFallback className="bg-gradient-to-br from-blue-100 to-purple-100 font-semibold text-blue-700 text-xs">
                {course.creator.name?.[0]}
              </AvatarFallback>
            </Avatar>
            <span className="font-medium text-gray-700 text-sm transition-colors duration-300 group-hover:text-gray-900">
              {course.creator.name}
            </span>
          </div>
